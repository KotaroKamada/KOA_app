...
def generate_pdf_report(player_name, section_scores, feedback_text, player_data, df, config):
    ...
    try:
        ...
        title_style = ParagraphStyle(
            ...
            fontSize=15,  # 16から15に変更（-1ポイント）
            spaceAfter=4,
            alignment=TA_CENTER, 
            textColor=colors.Color(0.1, 0.5, 0.2)  # 緑色に変更
        )
        ...
        score_table.setStyle(TableStyle([
            ...
            ('FONTSIZE', (0, 0), (-1, -1), 7),  # 9から7に変更（-2ポイント）
        ...
        if radar_chart:
            ...
            # レーダーチャート後のスペースを負の値で設定（上に押し上げ）
            story.append(Spacer(1, -2))  # さらに2ポイント上に移動
        ...
        for metric_key, metric_name, unit in key_metrics:
            ...
            if metric_key in body_composition_items:
                ...
                target_display = f"{format_value(target_val)}{unit}" if target_val is not None else "N/A"
        ...
        # 身体組成系項目の目標値セルを白で塗りつぶし
        row_index = 1  # ヘッダーの次の行から開始
        for metric_key, metric_name, unit in key_metrics:
            if metric_key not in df.columns:
                continue
            if metric_key in body_composition_items:
                # 目標値列（4列目、0ベース）を白で塗りつぶし
                table_style.append(('BACKGROUND', (4, row_index), (4, row_index), colors.white))
            row_index += 1
        
        detail_table.setStyle(TableStyle(table_style))
        
        story.append(detail_table)
        story.append(Spacer(1, 11))  # 12から11に変更
        ...
        try:
            ...
            story.append(Paragraph("○505テスト(前方スプリント) 5m直線スプリント後に方向転換するテストで、前方スプリント時の減速と再加速の能力を評価します。", item_style))
            story.append(Paragraph("○505テスト(バックペダル) 5m直線スプリント後にバックペダルで折り返すテストで、後方移動を含む多方向への移動能力を評価します。", item_style))
            ...
            story.append(Paragraph("○スクワットジャンプ・垂直跳び（反動あり）（SJ・CMJ） SJは下肢パワーを、CMJはパワーと反動利用能力を評価します。", item_style))
...
def create_triangle_radar_chart(section_scores, overall_score):
    ...
    try:
        ...
        # ラベルの追加（漢字に変更、文字化け対策として日本語フォントを試行）
        labels = ['身体組成', '俊敏性', '跳躍力']
        label_positions = [
        ...
        for i, (label, (x, y)) in enumerate(zip(labels, label_positions)):
            ...
            label_text.fontSize = 6  # 7から6に縮小（-1ポイント）
        ...
        total_text.fontSize = 8  # 9から8に縮小（-1ポイント）
            ...
            for player_name in all_players:
                try:
                    ...
                    if pdf_bytes:
                        ...
                        filename = f"KOA_Physical_Report_{safe_name}.pdf"  # 日付削除
        ...
        with col1:
            if st.button("📄 個人PDFレポート生成", type="primary"):
                with st.spinner('PDFレポートを生成中...'):
                    ...
                    if pdf_bytes:
                        filename = f"KOA_Physical_Report_{selected_name}.pdf"  # 日付削除
        ...
        with col2:
            if st.button("📁 全員分PDFレポート生成", type="secondary"):
                with st.spinner('全員分のPDFレポートを生成中...'):
                    ...
                    if all_pdf_bytes:
                        filename = f"KOA_All_Physical_Reports.zip"  # 日付削除
