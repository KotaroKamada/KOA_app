...
def generate_pdf_report(player_name, section_scores, feedback_text, player_data, df, config):
    ...
    try:
        ...
        try:
            ...
            story.append(Paragraph("○サイドホップ：片脚で10秒間に左右に何回ホップできるかを評価し、左右差から足関節の安定性を判断します。", item_style))
            ...
            for player_name in all_players:
                try:
                    ...
                    if pdf_bytes:
                        ...
                        filename = f"KOA_Physical_Report_{safe_name}.pdf"
        ...
        with col1:
            if st.button("📄 個人PDFレポート生成", type="primary"):
                with st.spinner('PDFレポートを生成中...'):
                    ...
                    if pdf_bytes:
                        filename = f"KOA_Physical_Report_{selected_name}.pdf"
        ...
        with col2:
            if st.button("📁 全員分PDFレポート生成", type="secondary"):
                with st.spinner('全員分のPDFレポートを生成中...'):
                    ...
                    if all_pdf_bytes:
                        filename = f"KOA_All_Physical_Reports.zip"
